...
describe "/api/v1/keyserver/lookup", :type => :api do
  ...
  context "lookup without params" do
    let(:url) {"api/v1/keyserver/lookup"}
    it "JSON" do
      get "#{url}"
      response.status.should eql(200)
      response.body.should eql('{"status":"error","message":"' + error_msg + '"}')
    end
  end
  ...
  context "lookup with email and expired key" do
    let(:url) {"api/v1/keyserver/lookup?email=#{expired_email}"}
    it "JSON" do
      get "#{url}"
      response.status.should eql(200)
      response.body.should eql('{"status":"error","message":"' + error_msg + '"}')
  ...
  context "lookup with email and unknown key" do
    let(:url) {"api/v1/keyserver/lookup?email=#{unknown_email}"}
    it "JSON" do
      get "#{url}"
      response.status.should eql(200)
      response.body.should eql('{"status":"success","keys":[{"key_id":"' + short_keyid + '","public_key":"' + public_key + '"}]}')
